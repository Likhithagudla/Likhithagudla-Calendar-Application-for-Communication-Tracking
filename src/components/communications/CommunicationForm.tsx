...
export function CommunicationForm({ onSubmit, onCancel, selectedCompanies }: CommunicationFormProps) {
  ...
  const { register, handleSubmit } = useForm<Partial<Communication>>();

  const handleFormSubmit = (data: Partial<Communication>) => {
    ...
    communications.forEach(onSubmit); // Submit each communication
    onCancel(); // Close the form/modal after submission
  };
